...
def nearest_straight_edge(square:Square, heading, lives:int) -> int:
...
def nearest_edge(square:Square):
    if square in edge_runs:
...
def isTunnel(location):
    ...
    for dx,dy in CORNERS:
        if gameMap.get_square(x + dx, y + dy).owner == myID:
            return False
    return True
...
def move(square:Square):
    ...
    if strength < square.production * 6 and strength < 100:
    ...
    if len(me) == 2 and isTunnel(square):
